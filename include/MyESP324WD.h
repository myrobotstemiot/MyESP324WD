...
//#include "SH1106.h"
#include "MyESP324WD_Motor_drive.h"
#include "MyESP324WD_Servo_lib.h"
#include "MyESP324WD_IO.h"
...
void MyESP324WD(){
  ...
  //display.drawString(25,0,"MyESP324WD");
  ...
  ledcAttachPin(M3A, 8);
  ledcAttachPin(M3B, 9);
  ledcAttachPin(M4A, 10);
  ledcAttachPin(M4B, 11);
...
int ADC_Read(int ADC_ch){
  return adc_mcp3008.readADC(ADC_ch);
}
void beep(){
  int _buzzer = 19;
...
{                
  ...
  {
    long value = map(ADC_Read(PID_SetupPin[i]), PID_Min[i], PID_Max[i], 100, 0);    
    if (value > 25) { 
      onLine = true;
    }
    if (value > 5){
